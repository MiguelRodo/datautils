...
#' @examples
#' set.seed(3)
#' test_tbl <- tibble(pid = rep(c('id1', 'id2', 'id1', 'id2'),  each = 2),
...
#'  test_tbl
#'
#'  filter_based_on_mean_grp_value(data = test_tbl, val = 'resp', grp_outer = 'type_base',
...
filter_based_on_mean_grp_value <- function(data,
  ...
  mean_tbl <- data %>%
    ...
    dplyr::summarise(val_mean = mean(!!ensym(val)),
                     .groups = 'drop')
