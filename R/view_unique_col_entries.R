...
view_cols <- function(x, n_entry = 20, n_entry_num = 5, n_col = 1e2,
  ...
  if(!is.numeric(n_entry)) stop(paste0("n_entry has class ", paste0(class(n_entry), collapse = " "),
                                       " when it should have class integer (or at least numeric"))
  if(!is.numeric(n_col)) stop(paste0("n_col has class ", paste0(class(n_col), collapse = " "),
                                     " when it should have class integer (or at least numeric"))
  if(!is.logical(silent)) stop(paste0("silent has class ", paste0(class(silent), collapse = " "),
  ...
  for(i in seq.int(1, n_col)){
    ...
    if(any(is.na(uni_vec))){
      ...
      } else disp_sample_vec <- uni_vec[1]
      # if none are NA, then display a random sample
    } else disp_sample_vec <- sample(uni_vec, min(n_entry_curr, length(uni_vec)), replace = FALSE)
    ...
    if(n_missing > 0 && !is.numeric(uni_vec)){
      ...
      } else print(paste0(n_missing, " unique entries not displayed"))
  ...
  if(!silent && n_col < ncol(x)){
    warning(paste0("Unique entries from only ", n_col, " of ", ncol(x),
                   " columns displayed"),
            call. = FALSE)
  }
