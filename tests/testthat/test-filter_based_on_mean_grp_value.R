test_that("filter_based_on_mean_grp_value works",{
  set.seed(3)
  test_tbl <- tibble::tibble(pid = rep(c('id1', 'id2', 'id1', 'id2'),  each = 2),
   ...
   dplyr::mutate(resp = purrr::map_dbl(type_base, function(x){
     round(rnorm(1, 5 + stringr::str_detect(x, "b") * 3), 2)})[1]) %>%
   dplyr::ungroup() %>%
   dplyr::mutate(resp = ifelse(stringr::str_detect(type, "_1"), rep(round(runif(4, 1e4, 1e5), 0), each = 2), 1) * resp)

   filter_tbl <- filter_based_on_mean_grp_value(data = test_tbl, val = 'resp', grp_outer = 'type_base',
                                                grp_inner = 'type', sel = 'smaller')

   correct_tbl <- tibble::tibble(pid = rep(c("id1", "id2"), 2),
